...
import { IPutReviewRequest } from "../types/requests/PutReview";
...
type Actions = {
  ...
  putReview: (id: string, body: IPutReviewRequest) => Promise<void>;
};
...
export const useReviewsStore = create<State & Actions>()(
  immer((set) => ({
    ...
    putReview: async (id: string, body: IPutReviewRequest) => {
      try {
        set({ isLoading: true, error: "" });
